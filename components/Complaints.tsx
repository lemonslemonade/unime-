...
const Complaints: React.FC = () => {
    ...
    const syncOfflineComplaints = useCallback(async () => {
        ...
        setIsSyncing(true);
        setSyncingIds(new Set(offlineFromStorage.map(c => c.id)));
        ...
        if (successfullySynced.length > 0) {
            ...
            toast.success(`${successfullySynced.length} offline complaint${successfullySynced.length > 1 ? 's' : ''} synced successfully.`);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isOnline) {
            ...
            setIsSubmitted(true);
            toast.info('Complaint saved locally. It will be submitted when you are back online.');
        }
    };
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                             ...
                             <div className="mt-4">
                                ...
                                <div className="mt-1">
                                    {attachedFile ? (
                                        <div className="text-sm bg-gray-50 p-3 rounded-md w-full flex items-center justify-between border border-gray-200">
                                            <div className="flex items-center truncate">
                                                ...
                                                <span className="font-semibold text-gray-800 truncate" title={attachedFile.name}>{attachedFile.name}</span>
                                                <span className="text-gray-500 ml-2 flex-shrink-0">({formatBytes(attachedFile.size)})</span>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={handleRemoveFile}
                                                className="text-gray-400 hover:text-brand-danger ml-2 p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-brand-danger"
                                    ...
                                    <input id="file-upload" ref={fileInputRef} name="file-upload" type="file" className="sr-only" onChange={handleFileChange} disabled={isLoading} />
                                </div>
                            </div>
                                ...
                                <div key={complaint.id} onClick={() => setSelectedComplaint(complaint)} className="bg-white p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex items-center space-x-4 border-l-4" style={{borderColor: styles.color}}>
                                    <div className="flex-grow">
                                        ...
                                        <div className="mt-3 text-xs text-gray-400 flex items-center">
                                            ...
                                            {syncingIds.has(complaint.id) && (
                                                 <svg className="animate-spin h-4 w-4 mr-1.5 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                </svg>
                                            )}
